...
{
    using Message = transport::Wrapper<Payload, Kind>;

    std::unique_ptr<Message> createMessage(std::string inResponseToId,
    ...
    std::unique_ptr<Message> createMessage(std::unique_ptr<Payload> payloadPtr)
    {
        auto id = transport::generateNewId();
